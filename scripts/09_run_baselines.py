#!/usr/bin/env python3
"""09_run_baselines.py -- Experiment E1 and the Gate 1 decision.

Gate 1 (`proposal/go_no_go.md`): PRISM-V's risk head must beat **every** naive temporal baseline at
`h in {1, 5, 10}` on at least two dataset families, where "beats" means the Holm-corrected paired
Wilcoxon test rejects at alpha = 0.05 **and** |Cliff's delta| >= 0.147.

If it fails, the project stops and the negative result is written up. Nothing in this script may be
tuned to change that outcome: the eps margin was fixed by the outcome-blind rule in D26 before this
was first run, the baselines are tuned on the calibration split with their budgets recorded, and
PRISM-V is early-stopped on the same calibration split using the task-matched metric.

Writes one JSON per (family, model seed) into results/e1_gate1/ plus a run manifest.
"""
...
def run_family(family: str, entries: list[dict], model_seed: int, log) -> dict:
    ...
    for variant in ("two_head", "first_passage"):
        ...
        log(f"     {variant}: best_epoch={m.best_epoch}")
    ...
    for variant in ("two_head", "first_passage"):
        ...
        out["gate1"][mk] = {
            "per_horizon": per_h,
            "passes_this_family": bool(all(v["all_beaten"] for v in per_h.values())),
        }
    return out


def main() -> int:
    ap = standard_parser("E1 / Gate 1: risk-path forecasting vs naive temporal baselines",
                         require_config=False)
    ap.add_argument("--scale", default="full", choices=["smoke", "dev", "full"])
    args = ap.parse_args()

    out_dir = Path(args.out or "results/e1_gate1")
    ensure_dir(out_dir)
    log = get_logger("gate1", args.log or "logs/gate1.log").info
    ...
    for family, entries in sorted(by_family.items()):
        ...
        for seed in seeds:
            ...
            if should_skip(target, args.force, logger=None):
                ...
                continue
            with run_manifest(f"E1_gate1_{family}_seed{seed}", out_path=out_dir /
    ...
    for variant in ("two_head", "first_passage"):
        ...
        for family in sorted(f for f in by_family if f not in excluded):
            ...
            if not rows:
                continue
            passes = [r["gate1"][mk]["passes_this_family"] for r in rows]
            beaten = {f"h{h}": [r["gate1"][mk]["per_horizon"][f"h{h}"]["n_beaten"] for r in rows]
                      for h in GATE1_HORIZONS}
        ...
        verdict[f"{mk}_GATE1"] = "PASS" if n_fam >= 2 else "FAIL"
        log(f"  -> {mk}: {n_fam} family/families pass; GATE 1 = "
            f"{'PASS' if n_fam >= 2 else 'FAIL'}\n")

    write_json(verdict, out_dir / "GATE1_VERDICT.json")
    log(f"verdict written to {out_dir / 'GATE1_VERDICT.json'}")
    return 0
